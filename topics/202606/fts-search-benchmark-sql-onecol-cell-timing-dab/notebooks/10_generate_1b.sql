-- Databricks notebook source
    ...
    --   fts_text_1b_scan   : no full-text index, baseline scan
    --   fts_text_1b_ngram  : ngram full-text index target for substring tests
    --   fts_text_1b_split  : split full-text index target for word tests
    ...
    CREATE OR REPLACE TABLE fts_text_1b_scan
    TBLPROPERTIES (
  ...
  'orders','payments','profile','workflow','jobs','governance','lineage','quality','monitoring','observability'
      ...
      SELECT
        concat_ws(' ',
                    CASE WHEN id = 999999999 THEN 'zztailneedlealpha zztailwordprobe' END,
CASE WHEN pmod(id, 100000) = 42 THEN 'zzqneedlealpha zztokenomega' END,
    ...
    ANALYZE TABLE fts_text_1b_split COMPUTE STATISTICS;

    INSERT INTO fts_benchmark_object_checks
    SELECT :run_id, current_timestamp(), '1b', 'fts_text_1b_scan', 'table', 'row_count', CAST(COUNT(*) AS STRING), '1000000000', CASE WHEN COUNT(*) = 1000000000 THEN 'PASS' ELSE 'FAIL' END
    FROM fts_text_1b_scan
    UNION ALL
    SELECT :run_id, current_timestamp(), '1b', 'fts_text_1b_ngram', 'table', 'row_count', CAST(COUNT(*) AS STRING), '1000000000', CASE WHEN COUNT(*) = 1000000000 THEN 'PASS' ELSE 'FAIL' END
    FROM fts_text_1b_ngram
    UNION ALL
    SELECT :run_id, current_timestamp(), '1b', 'fts_text_1b_split', 'table', 'row_count', CAST(COUNT(*) AS STRING), '1000000000', CASE WHEN COUNT(*) = 1000000000 THEN 'PASS' ELSE 'FAIL' END
    FROM fts_text_1b_split;

    SELECT 'fts_text_1b_scan' AS table_name, COUNT(*) AS row_count FROM fts_text_1b_scan
    UNION ALL SELECT 'fts_text_1b_ngram', COUNT(*) FROM fts_text_1b_ngram
    UNION ALL SELECT 'fts_text_1b_split', COUNT(*) FROM fts_text_1b_split;
