...
CREATE SEARCH INDEX ftsbench_si_10m_message_ngram_v1
ON fts_text_10m_ngram (message)
OPTIONS (tokenizer = 'ngram');

-- Word test index: split tokenizer, default min_token_length.
CREATE SEARCH INDEX ftsbench_si_10m_message_split_v1
ON fts_text_10m_split (message)
...
OPTIMIZE fts_text_10m_scan;
OPTIMIZE fts_text_10m_ngram;
OPTIMIZE fts_text_10m_split;

REFRESH INDEX ftsbench_si_10m_message_ngram_v1 FULL;
REFRESH INDEX ftsbench_si_10m_message_split_v1 FULL;
...
WHERE table_catalog = :catalog
  ...
  AND table_name IN ('fts_text_10m_scan','fts_text_10m_ngram','fts_text_10m_split')
GROUP BY table_name;

DESCRIBE INDEX ftsbench_si_10m_message_ngram_v1;
DESCRIBE INDEX ftsbench_si_10m_message_split_v1;

SELECT *
FROM fts_benchmark_object_checks
WHERE run_id = :run_id AND scale_label = '10m'
